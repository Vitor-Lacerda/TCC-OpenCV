				...
				*/
				...
				if (cont > 10) {
					for (int i = 0; i < vagas.size(); i++)
					{
						if (vagas[i].ocupada) {
							stringstream oss;
							oss <<"../ImagensTeste/Extraidas/Carros/Carros"  << numeroCarro << ".png";
							imwrite(oss.str(), quadro(vagas[i].limites));
							numeroCarro++;

						}
						else {
							stringstream oss;
							oss << "../ImagensTeste/Extraidas/Vagas/Vagas" << numeroVaga << ".png";
							imwrite(oss.str(), quadro(vagas[i].limites));
							numeroVaga++;
						}
					}
...
void VerificadorVagas::VerificaEstado(Mat quadro, Mat fundo,  vector<Vaga> &vagas)
{
	...
	for (int i = 0; i < vagas.size(); i++) {
		...
		vagas[i].ocupada = DeterminaOcupacao(imgVaga,i);
